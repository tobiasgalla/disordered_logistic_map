...
omvec = np.linspace(-np.pi,np.pi*(numsample-1.0)/numsample, numsample, endpoint = True)
...
def trial(tr):
    ...
    while t <= tmax + numsample:
        ...
        for i in range(0, numsample):
            for j in range(0, n):
                transform[i][j] = transform[i][j] + np.exp((0.0+1.0j)*omvec[i]*(step))*(x[j]-avdet[j])
...
if __name__ == '__main__':
    ...
    mystr = "_n4000_numtrial10_numsample1000_r1point2_sigmasqrt2point1.csv"
